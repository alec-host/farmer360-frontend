...
import { BUSINESS_LIMITED_SCOPE_KEY, readLocalCache, storeOnLocalCache } from "../../../../db/localSessionData";
...
const AdminBusinessPage = () => {
    ...
    const [storeBusinessData, setStoreBusinessData] = useState([]);
    ...
    useEffect(() => {
        const stored_data = readLocalCache(BUSINESS_LIMITED_SCOPE_KEY);
        if(stored_data){
            setStoreBusinessData(stored_data);
        }
    },[]);

    const filteredItems = storeBusinessData.filter(
        item =>
          JSON.stringify(item)
            .toLowerCase()
    ...
    const handleRefresh = () => {
        ...
        getBusinessUserList(1);
        Loading.remove(1523);
    };

    const getBusinessUserList = async(reference_number) => {
        const config = {
        method: 'GET',
        url: API_END_POINT+'/api/v1/adminGetBusinesslimitedScope?email=admin&reference_number='+reference_number,
        headers: { 
          'Content-Type': 'application/json'
        }};
        await axios(config).then((resp) => {
          if(resp){
            storeOnLocalCache(BUSINESS_LIMITED_SCOPE_KEY,resp?.data?.data);
            setStoreBusinessData(resp?.data?.data);
          }
        })
        ...
        .then(async(response) => {
            await response.json().then(data=>{
                if(data?.success){
                    ...
                    storeOnLocalCache(BUSINESS_LIMITED_SCOPE_KEY,data?.data); 
                    setTimeout(() => {
    ...
    const actionsMemo = React.useMemo(() => <div><Export onExport={() => downloadCSV(filteredItems,filteredItems)} /><Refresh onRefresh={handleRefresh}/></div>, [filteredItems,storeBusinessData]);
                ...
                <div className="row" style={{marginTop:"15px"}}>
                    <div className="content">
                        <table style={{width:"100%"}}>
                            ...
                            <tbody>
                                <tr><td><h5><strong>Businesses</strong></h5></td></tr>
