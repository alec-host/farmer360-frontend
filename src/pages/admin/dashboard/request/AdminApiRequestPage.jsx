...
import { API_REQUEST_KEY, readLocalCache, storeOnLocalCache } from "../../../../db/localSessionData";
...
const AdminApiRequestPage = () => {
    ...
    useEffect(() => {
        const stored_data = readLocalCache(API_REQUEST_KEY);
        if(stored_data){
    ...
    const handleRefresh = () => {
        ...
        getApiServiceRequest(1);
        Loading.remove(1523);
    };

    const getApiServiceRequest = async(reference_number) => {
        const config = {
        method: 'GET',
        url: API_END_POINT+'/api/v1/adminGetAllServiceRequests?request_type=api&reference_number='+reference_number,
        headers: { 
          'Content-Type': 'application/json'
        }};
        await axios(config).then((resp) => {
          if(resp){
            storeOnLocalCache(API_REQUEST_KEY,resp?.data?.data);
    ...
    const columns = [   
        ...
        {
            name: makeTextBold('DESCRIPTION'),
            selector: row => <><i>{row.description}</i></>,
            sortable: false,
            right: false,
            allowOverflow: true,
            wrap: 'yes',
                ...
                <div className="row" style={{marginTop:"15px"}}>
                    <div className="content">
                        <table style={{width:"100%"}}>
                            ...
                            <tbody>
                                <tr><td><h5><strong>API Integration Requests</strong></h5></td></tr>
